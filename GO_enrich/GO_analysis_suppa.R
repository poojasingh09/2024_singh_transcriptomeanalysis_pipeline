...
GO2geneID <-readMappings(file = "/Users/singhpoo/Desktop/Papers/2019_postdoc_papers/2019_RNAseq_newgrant/2023_allst26/analysis/new_ref/10_GOenrichment/DTU/goframeData.txt")
...
gene.universe.exp <-read.table("/Users/singhpoo/Desktop/Papers/2019_postdoc_papers/2019_RNAseq_newgrant/2023_allst26/analysis/new_ref/10_GOenrichment/DTU/gene_universe.txt",header=F)
...
files = list.files(path=".", pattern=".mstrg_id.ensembl")
...
genesOfInterest <- as.character(sig_genes$V2)

# only my universe from the RNAseq experiment
geneList <- factor(as.integer(geneNames.universe %in% as.character(sig_genes$V2)))
